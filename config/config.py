...
config = edict()
config.dataset = "emoreIresNet" #"webface" #emoreIresNet" # training dataset
config.embedding_size = 512 # embedding size of model
config.momentum = 0.9
config.weight_decay = 5e-4
config.batch_size = 128 # batch size per GPU
config.lr = 0.1
config.output = "/home/bringascastle/Escritorio/ElasticFace/output/R100_ElasticArcFace" # train model output folder
...
if config.dataset == "emoreIresNet":
    config.rec = "/home/bringascastle/Escritorio/ElasticFace/faces_emore"
    config.num_classes = 85742
    config.num_image = 5822653
    config.num_epoch =  26
    config.warmup_epoch = -1
    config.val_targets =  ["calfw"] #"lfw", "cfp_fp", "cfp_ff", "agedb_30", "calfw", "cplfw"]
    ...
    config.rec = "/content/drive/MyDrive/Colab Notebooks/ElasticFace/data/fboutros/faces_webface_112x112"
